...
greenbook/
  ...
  schedules/
    gb/2026-01-01.toml       - the current GB schedule (lifted from spec/standard.md)
  products/
    gb-snomed-dm.toml        - SNOMED UK drug extension product → antigens map
  tests/
...
cargo run --quiet --bin greenbook -- \
  evaluate \
  schedules/gb/2026-01-01.toml \
  products/gb-snomed-dm.toml \
...
- `Overall status: PARTIALLY_VACCINATED` — see [queries.md](../queries.md) §2 for why this is technically correct but clinically misleading for an on-schedule infant.
...
You'll also see some `INVALID` doses listed under Hib/MenC and Td/IPV — those are the 6-in-1 doses being re-matched into booster series via antigen overlap (Hib; tetanus/diphtheria/polio). This is a real design question — see [queries.md](../queries.md) §1.
...
| Change a vaccineCode to a SNOMED code not in `products/gb-snomed-dm.toml` | The dose silently disappears from the evaluation — no series matches it. (Worth adding an "unknown product code" warning in a follow-up.) |

## 8. Inspecting the schedule itself

Open `schedules/gb/2026-01-01.toml` directly. Every series is one `[[series]]` block with its doses inline; antigen IDs at the bottom map to SNOMED concept codes. Editing this file (and re-running step 5) is how you would propose a schedule change.

## 9. Next steps

See [queries.md](../queries.md) for the open design questions the POC raised. The biggest one (§1, dose-to-series matching) probably wants resolving before more fixtures are added.
