...
- [ ] **One product class shared by several series — dose allocation.** *Discovered via the web demo (a child with both MMR doses).* When a class maps to more than one series (currently only `MMR` → `mmr-primary` + `mmr-second`), the engine matches each dose against *every* series of that class independently, so an on-time MMR-dose-1 + MMR-dose-2 record produces spurious "extra dose" / "given before earliest_age" flags. Doses of a shared class should instead be **allocated across that class's series** (each dose fills one slot, in order), treating the class as one programme. This is a design decision (e.g. how interval-from-previous spans series); a [glossary note](./ubiquitous-language.md) already flags "one class can serve several series". Fix in both the Rust engine and the JS port, re-validate, and add a both-MMR-doses fixture.
