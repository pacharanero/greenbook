...
This changelog tracks the **crate and tooling**. Changes to schedule *content* are recorded per-version in each schedule file's `change_summary` and in the git history of `rules/`.

## [Unreleased]

### Changed

- **Repository restructured into peer implementations.** The Rust crate moved from the repo root to [`rust/`](./rust/); the JavaScript engine moved from `docs/demo/engine.js` to [`js/`](./js/) as a first-class implementation (UMD module + FHIR parser + tests). The canonical sources (`rules/`, `spec/`) stay top-level. A new [`conformance/`](./conformance/) suite (shared fixtures + a case manifest + committed golden outputs) is the single mechanism keeping implementations in step: Rust generates the goldens; every implementation's tests assert it reproduces them, so each is validated standalone. The demo (`docs/demo/`) now vendors a generated copy of the JS engine. CI runs both a Rust job and a JS job. More implementations (Ruby, Python) can join by running the same suite. (FHIR test-data fixtures moved from `tests/fixtures/` to `conformance/fixtures/`.)
- Conformance matching is now by **product class** rather than antigen overlap, separating "schedule conformance" from "antigen coverage" ([conformance vs coverage](./spec/conformance-vs-coverage.md)). Product map entries and schedule series now carry a required `product_class` field. This fixes 6-in-1 doses being falsely flagged INVALID under the Hib/MenC and Td/IPV booster series.
- Schedule and product-map files live together in a single top-level [`rules/`](./rules/) directory, distinguished by filename prefix (`schedule-uk-2026-01-01.toml`, `product-map-uk-snomed-dm.toml`) so each kind globs cleanly and the jurisdiction stays in the name. The jurisdiction code is set to `UK` (ISO 3166 exceptionally-reserved, chosen over `GB` so the UK-wide scope incl. Northern Ireland is unambiguous).
...
- Current UK schedule (`rules/schedule-uk-2026-01-01.toml`) and UK SNOMED drug-extension product map (`rules/product-map-uk-snomed-dm.toml`).
